...
const chartData = computed(() => {
  // 상품명이 너무 길면 잘라서 표시
  const labels = userProducts.value.map(item => {
    const name = item.product.fin_prdt_nm
    return name.length > 8 ? name.substring(0, 8) + '...' : name
  })

  // 기본 금리 데이터
  const baseRates = userProducts.value.map(item => item.option?.intr_rate || 0)
  // 최고 우대 금리 데이터
  const maxRates = userProducts.value.map(item => item.option?.intr_rate2 || 0)
...
const chartOptions = {
  ...
  plugins: {
    legend: {
      position: 'bottom',
      labels: {
        color: '#6b7280', // 범례 텍스트 색상 (Gray-500)
  ...
  scales: {
    y: {
      beginAtZero: true,
      grid: {
        color: 'rgba(200, 200, 200, 0.1)' // 그리드 라인 투명도 조절
      },
      ticks: {
        color: '#9ca3af', // Y축 글자 색상
...
<template>
  <div class="w-full min-h-screen flex flex-col relative overflow-hidden text-primary font-pretendard transition-colors duration-300">
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-4 pt-32 pb-12 z-10">
      ...
      <div v-else class="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
        
        <div v-if="userProducts.length > 0" class="glass-panel p-6 rounded-2xl shadow-xl border-line bg-indigo-50/50 dark:bg-white/5">
        ...
        <div class="glass-panel rounded-2xl overflow-hidden shadow-xl border-line">
          
          <div class="grid grid-cols-12 gap-4 p-5 border-b border-gray-100 dark:border-gray-700 bg-indigo-50/50 dark:bg-white/5 text-sm font-medium text-secondary">
          ...
          <div v-if="userProducts.length > 0">
            <div 
              ...
              :key="item.id"
              class="grid grid-cols-12 gap-4 p-5 items-center border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-indigo-50/30 dark:hover:bg-white/5 transition-colors cursor-pointer group"
