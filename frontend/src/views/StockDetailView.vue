  ...
  <div class="w-full min-h-screen flex flex-col relative overflow-hidden text-primary font-pretendard transition-colors duration-300">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
        ...
        <div class="glass-panel rounded-2xl p-6 md:p-8">
          ...
          <div class="grid grid-cols-2 gap-y-8 gap-x-4" v-if="stockStore.currentStock.latest_price">
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">전일 종가</p>
              <p class="text-lg font-bold text-primary">{{ formatNum(prevClose) }}원</p>
            </div>
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">시가 (Open)</p>
            ...
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">고가 (High)</p>
              <p class="text-lg font-bold text-rose-500 dark:text-rose-400">{{ formatNum(stockStore.currentStock.latest_price.high_price) }}원</p>
            </div>
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">저가 (Low)</p>
              <p class="text-lg font-bold text-blue-600 dark:text-blue-400">{{ formatNum(stockStore.currentStock.latest_price.low_price) }}원</p>
            </div>
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">거래량</p>
              <p class="text-lg font-bold text-primary">{{ formatNum(stockStore.currentStock.latest_price.volume) }}주</p>
            </div>
            <div class="group">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">거래대금</p>
              <p class="text-lg font-bold text-primary">{{ formatNum(Math.floor(stockStore.currentStock.latest_price.trading_value / 1000000)) }}백만</p>
            </div>
            <div class="group col-span-2 border-t border-line pt-4 mt-2">
              <p class="text-secondary text-sm mb-1 group-hover:text-primary transition-colors">시가총액</p>
              <p class="text-lg font-bold text-primary">{{ formatNum(Math.floor(stockStore.currentStock.market_cap / 100000000)) }}억</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-indigo-50/80 dark:bg-indigo-900/20 backdrop-blur-md border border-indigo-200 dark:border-indigo-500/30 rounded-2xl p-6 md:p-8 flex flex-col justify-between relative overflow-hidden">
          <div class="absolute inset-0 bg-gradient-to-br from-indigo-100/50 dark:from-indigo-600/10 to-transparent -z-10"></div>
          
          <div>
            <div class="flex justify-between items-start mb-6">
              <h3 class="text-xl font-bold text-indigo-700 dark:text-indigo-300 flex items-center gap-2">
          ...
          <button class="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-indigo-500/30 active:scale-95 flex items-center justify-center gap-2 group">
            AI 분석 리포트 생성하기
    ...
    <div v-else class="flex-1 flex items-center justify-center min-h-[60vh]">
      <div class="flex flex-col items-center gap-4">
        ...
        <p class="text-secondary animate-pulse">데이터를 불러오는 중...</p>
      </div>
    </div>
